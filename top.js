...
function showDashboard() {
  ...
  var html = `
    <div style='padding: 8px;' id='zrr-login-container'>
      ...
      <div><input type="text" name="zrr-email" id="zrr-email" style='width: 100%; font-size: 16px; padding: 4px;' /></div>
      <div style='text-align: center; padding: 8px;'>Password:</div>
      <div><input type="password" name="zrr-password" id="zrr-password" style='width: 100%; font-size: 16px; padding: 4px;' /></div>
      <div style='text-align: center; padding: 16px;'>
      <input type="submit" name="zrr-login-btn" id="zrr-login-btn" value="Login" style='font-size: 16px; padding: 4px; width:70px' />
      <input type="submit" name="zrr-signup-btn" id="zrr-signup-btn" value="SignUp" style='font-size: 16px; padding: 4px; width:70px' />
      </div>
    </div>
...
function updateDashboard(msg) {
  ...
  if (betFavorites == null) {
    ...
    document.getElementById('zrr-recommendation').innerHTML = "Zen Roulette Recommendation";
...
function signupZRR() {
  window.open('https://www.zenroulette.com/zen-roulette-assistant/');
}
...
async function loginZRR() {
  ...
  try {
    ...
    if (msg.success == true) {
      loggedIn = true;
      if (msg.membership == 'trial') {
        $('#zrr-membership').html(`
          <div>${email}</div>
          <div>Trial Membership</div>
        `);
      } else { // zrr membership
        $('#zrr-membership').html(`
          <div>${email}</div>
          <div>ZRR Membership</div>
        `);
      }
